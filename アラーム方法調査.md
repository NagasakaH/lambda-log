...
```mermaid
flowchart LR
    Lambda[Lambda] --> LG[CloudWatch Logs]
    LG --> MF[Metric Filter]
    MF --> Metric[CloudWatch Metrics]
...
```mermaid
flowchart LR
    Lambda[Lambda with Powertools] -->|EMF JSON| LG[CloudWatch Logs]
    Lambda -.->|Auto Publish| Metric[CloudWatch Metrics]
    Metric --> Alarm[CloudWatch Alarm]
    Alarm --> SNS[SNS Topic]
```
...
flowchart LR
    Lambda[Lambda] --> LG[CloudWatch Logs]
    LG --> AD[Log Anomaly Detector]
    AD --> Alarm[CloudWatch Alarm]
    Alarm --> SNS[SNS Topic]
```
...
```mermaid
flowchart TB
    subgraph Alarms[Individual Alarms]
...
flowchart LR
    ...
    AlertLambda --> External[External Services]
```
